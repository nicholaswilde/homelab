...
################################################################################
#
# Script Name: update-reprepro-service.sh
# ----------------
# Service-friendly version of update-reprepro.sh.
# Downloads application tar.gz and .deb files, packages them as needed, and
# adds them to a reprepro repository.
# Designed to be run from a non-interactive service like a webhook.
#
...
function update_app_from_source() {
  ...
  export DESCRIPTION=$(curl -s "https://api.github.com/repos/${GITHUB_REPO}" | jq -r '.description' | sed -e 's/:[[:alnum:]_:]//g' -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')
...
function update_tea() {
  ...
  export DESCRIPTION=$(curl -s "https://gitea.com/api/v1/repos/${GITHUB_REPO}" | jq -r '.description' | sed -e 's/:[[:alnum:]_:]//g' -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')
