...
# ----------------
# Checks for the latest release of bentopdf and compares it to
...
GITHUB_REPO="alam00000/bentopdf"
DEBUG="false"
...
function check_dependencies() {
  if ! command_exists curl || ! command_exists jq || ! command_exists npm || ! command_exists tar; then
    log "ERRO" "Required dependencies (curl, jq, npm, tar) are not installed." >&2
    exit 1
  fi
}
...
function main() {
  ...
  log "INFO" "Extracting..."
  # Extract to INSTALL_DIR, stripping the top level directory
  ...
  cd "${INSTALL_DIR}"
  if ! npm ci --no-audit --no-fund 2>&1 | log "INFO"; then
      log "ERRO" "npm ci failed."
      exit 1
  fi
  
  log "INFO" "Building application..."
  export SIMPLE_MODE=true
  if ! npm run build -- --mode production 2>&1 | log "INFO"; then
